...
import { globalComponents, localComponents } from "#smile:components"
...
<template>
  <MDCRenderer v-if="!isEmpty" :body="body" :data="data" :class="props.class" :tag="props.tag" :prose="props.prose"
    :unwrap="props.unwrap" :components="componentsMap" :data-content-id="debug ? value.id : undefined" />
  <slot v-else name="empty" :body="body" :data="data" :data-content-id="debug ? value.id : undefined">
    <!-- nobody -->
  </slot>
</template>
